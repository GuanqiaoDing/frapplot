#' Process FRAP data
#'
#' @description Takes quantified data from ImageJ, perform calculations and non-linear curve fitting. The raw data is organized as below:
#'  * There is a main folder
#'  * Each group (control or experimental) is in a subfolder
#'  * In each subfolder, there are n files for individual samples where n = sample size
...
#' @return The function has no return value. Instead, it generates output files:
...
frapprocess <- function(rawdata_dir, time_points = seq(0, 145, 5)) {
  ...
  #(sub)folder_names means sample names, eg: Dcp2
  folder_names <- dir()
  ...
  for (i in 1 : folder_num){
    ...
    #record the results in FRAPdata
